...
<?php if(!isset( $_SESSION['is_change_phone'] ) ){  ?>
    <div id="phone-number"  <?php if(isset( $_SESSION['notset'] )){ echo 'style="display:block"'; } else{ echo 'style="display:none"'; }?> >
        <?php echo form_open('/f/subscriber/login_verification', ['id' => 'subscriber_form', 'class' => 'needs-validation', 'novalidate'=>'']); ?>
...
timer = setInterval(function() {
	if( count > 1 ){
		...
		var index = '<?php echo encryption("send_new_code")?>';
        var url = '<?php echo base_url()."f/subscriber/login_verification?resend_otp="?>'+index;
