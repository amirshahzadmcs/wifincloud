...
class Login extends CI_Controller {
	...
	public function check()
	{
        ...
        if( $attempt=='valid' ){
			...
			if (get_role_name($user->role) == 'Business Owner') {
				// Fetch domain details based on user ID
				$domain = $this->db->where('users_id', $user->id)->get($this->domains_model->table)->row();
				$isExpired = licenceDateColor($domain->license_expiry_date);

				if ($isExpired['isExpired']) {
					$this->session->set_flashdata('message', 'Your domain has expired. Please contact the administrator to renew your domain.');
	...
	public function email_verification($id)
	{
		...
		if(isset( $_GET['domain'] )){
			...
			$domain_name = decryption($domain);
			 switch_db( $domain_name);
			$exists = $this->db->get_where($table , ['verification_id' =>  $id] )->first_row();
			if( isset($exists->id)){
				$table = "subscribers";
				$this->db->where('email',  $exists->email);
				$this->db->update($table , ['email_verify' => 1]);
